...
from .bandgap import analyze_bandgap
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if dos_data:
                ...
                # 将原始数组序列化后存入 analysis_data，供 HTML 生成器生成下载链接
                raw_dos_serializable = {}
                for key, val in dos_data.items():
                    raw_dos_serializable[key] = {k2: (v2.tolist() if hasattr(v2, 'tolist') else list(v2)) for k2, v2 in val.items()}
                self.analysis_data['raw_dos_data'] = raw_dos_serializable
...
class PyMatGenDOSHTMLGenerator(BaseHTMLGenerator):
    """PyMatGen DOS分析结果的HTML报告生成器"""

    # ---------- 子类覆写的属性 ----------
    report_title: str = "DOS 分析报告"
    report_emoji: str = "🔬"
    report_heading: str = "PyMatGen DOS 分析报告"
    report_attribution: str = "PyMatGen DOS分析报告 | 基于Materials Project生态系统"
    ...
    def _get_extra_css_styles(self) -> str:
        """DOS-specific styles (highlight, warning, info-box, etc.)"""
        return """
        .section h3 {
            color: #764ba2;
    ...
    # ------------------------------------------------------------------ #
    #  JavaScript (none needed for DOS — uses base64 images)
    # ------------------------------------------------------------------ #
    ...
    def _dos_csv_link(self, raw_dos_data: Dict[str, Any], keys: List[str], filename: str) -> str:
        ...
        if not raw_dos_data:
            return ''
        # 收集所有 key 的数据，以 TDOS.energy 作为公共能量轴
        energy = None
        columns: Dict[str, list] = {}
        for k in keys:
            entry = raw_dos_data.get(k)
            if not entry:
                continue
            if energy is None and 'energy' in entry:
                energy = entry['energy']
            for col_name, col_vals in entry.items():
                if col_name == 'energy':
                    continue
                columns[f'{k}_{col_name}'] = col_vals
        if energy is None:
            return ''
        headers = ['能量(eV相对费米能级)'] + list(columns.keys())
        rows = []
        for i, e in enumerate(energy):
            row = [e] + [columns[c][i] if i < len(columns[c]) else '' for c in columns]
            rows.append(row)
        lines = [','.join(str(v) for v in headers)]
        for row in rows:
            lines.append(','.join('' if v == '' else str(round(v, 8) if isinstance(v, float) else v) for v in row))
        csv_b64 = base64.b64encode('\n'.join(lines).encode('utf-8')).decode('ascii')
        return (
            f'<div class="download-bar">'
            f'<a href="data:text/csv;base64,{csv_b64}" download="{filename}" class="download-link">📥 下载原始数据 (CSV)</a>'
            f'</div>'
        )
    ...
    def _generate_visualization_section(self, charts_data: Dict[str, Any]) -> str:
        ...
        raw = self.data.get('raw_dos_data', {})
        all_keys = list(raw.keys())
        elem_keys = [k for k in all_keys if k != 'TDOS']

        tdos_dl = self._dos_csv_link(raw, ['TDOS'], 'total_dos.csv')
